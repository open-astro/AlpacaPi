...
#*	Override OpenCV variables if OpenCV is not installed
#*	This prevents pkg-config errors when OpenCV is missing
EOF
	
	#*	Set OpenCV variables to empty if OpenCV is not detected
	if [ "$OPENCV_V3_OK" = false ] && [ "$OPENCV_V4_OK" = false ]
	then
		cat >> "$CUSTOM_MAKEFILE" << 'EOF'
OPENCV_VERSION		=
...
EOF
	...
	then
		make clean >/dev/null 2>&1
		#*	Capture build output to show errors if it fails
