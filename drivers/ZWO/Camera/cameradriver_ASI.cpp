...
//*****************************************************************************
TYPE_ASCOM_STATUS	CameraDriverASI::StartPulseGuide(const TYPE_GuideDirections direction, const int durationMilliseconds, char *alpacaErrMsg)
{
TYPE_ASCOM_STATUS	alpacaErrCode	=	kASCOM_Err_Success;
ASI_ERROR_CODE		asiRetCode;
ASI_GUIDE_DIRECTION	asiDirection;

	CONSOLE_DEBUG(__FUNCTION__);
	if (!cSt4Port)
	{
		alpacaErrCode	=	kASCOM_Err_NotImplemented;
		GENERATE_ALPACAPI_ERRMSG(alpacaErrMsg, "Camera does not have ST4 port");
		return(alpacaErrCode);
	}

	//*	Convert Alpaca direction to ZWO SDK direction
	switch(direction)
	{
		case kGuide_North:
			asiDirection	=	ASI_GUIDE_NORTH;
			break;
		case kGuide_South:
			asiDirection	=	ASI_GUIDE_SOUTH;
			break;
		case kGuide_East:
			asiDirection	=	ASI_GUIDE_EAST;
			break;
		case kGuide_West:
			asiDirection	=	ASI_GUIDE_WEST;
			break;
		default:
			alpacaErrCode			=	kASCOM_Err_InvalidValue;
			GENERATE_ALPACAPI_ERRMSG(alpacaErrMsg, "Invalid direction");
			return(alpacaErrCode);
	}

	//*	Call ZWO SDK function to start pulse guiding
	asiRetCode	=	ASIPulseGuideOn(cAsiCameraInfo.CameraID, asiDirection);
	...
	{
		alpacaErrCode	=	kASCOM_Err_UnspecifiedError;
		Get_ASI_ErrorMsg(asiRetCode, alpacaErrMsg);
		CONSOLE_DEBUG_W_NUM("ASIPulseGuideOn() returned\t=", asiRetCode);
	}
	return(alpacaErrCode);
}

//*****************************************************************************
TYPE_ASCOM_STATUS	CameraDriverASI::StopPulseGuide(const TYPE_GuideDirections direction, char *alpacaErrMsg)
{
TYPE_ASCOM_STATUS	alpacaErrCode	=	kASCOM_Err_Success;
ASI_ERROR_CODE		asiRetCode;
ASI_GUIDE_DIRECTION	asiDirection;
	...
	//*	Call ZWO SDK function to stop pulse guiding
	asiRetCode	=	ASIPulseGuideOff(cAsiCameraInfo.CameraID, asiDirection);
	if (asiRetCode == ASI_SUCCESS)
	{
		alpacaErrCode	=	kASCOM_Err_Success;
	}
	else
	{
		alpacaErrCode	=	kASCOM_Err_UnspecifiedError;
		Get_ASI_ErrorMsg(asiRetCode, alpacaErrMsg);
		CONSOLE_DEBUG_W_NUM("ASIPulseGuideOff() returned\t=", asiRetCode);
	}
	return(alpacaErrCode);
}
